                                  ...
                                  bool drop_broad_sweep)
{
    ...
    // v6.2 instrumentation regions (schema v2):
    // Region 6: phys_module body (world position leaf at +0x70)
    if (s.phys_module) {
        if (WriteRegionRecord(w, REGION_PHYS_MODULE,
                              s.phys_module, 6u,
                              0, REGION_CAP_PHYS_MODULE,
                              false, 0)) ++count;
        else return count;
    }

    // Region 7: action_request module body (Erd-Tools enemy anim_id source)
    if (s.action_request) {
        if (WriteRegionRecord(w, REGION_ACTION_REQUEST,
                              s.action_request, 7u,
                              0, REGION_CAP_ACTION_REQUEST,
                              false, 0)) ++count;
        else return count;
    }
    ...
    if (s.time_act) {
        ...
        for (int off = R8_SCAN_START; off < TIME_ACT_CHILD_SCAN_BYTES &&
            ...
            if (!LooksLikeUserPtrFast(target)) continue;
            if (WriteRegionRecord(w, REGION_TIME_ACT_CHILD_BODY,
                                  target, 8u,
                                  0, REGION_CAP_TIME_ACT_CHILD_BODY,
                                  true, (uint16_t)off)) {
                ++count;
                ++childN;
            } else {
